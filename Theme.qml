...
Item {
    ...
    readonly property string colorsPath: Quickshell.env("HOME") + "/.config/omaniri/current/theme/colors.toml"
    ...
    property color accentHint: sealRaw
    ...
    // iwd (Omarchy 3.8.x) → impala/bluetui through omaniri-launch-*; if NetworkManager
    // is the active backend (Omarchy 4.0) → nmtui instead.
    readonly property string launchWifiCmd: "if systemctl is-active --quiet NetworkManager 2>/dev/null; then omaniri-launch-or-focus-tui nmtui; else omaniri-launch-wifi; fi"
    readonly property string launchBtCmd:   "omaniri-launch-bluetooth"
    ...
    // ── Niri workspace dispatch ──
    Process { id: wsFocusProc }
    function gotoWorkspace(id) {
        if (id === undefined || id === null) return
        wsFocusProc.command = ["niri", "msg", "action", "focus-workspace", String(id)]
