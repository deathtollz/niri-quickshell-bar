...
Item {
    ...
    // "stay awake" mode active when idle daemon is NOT running
    ...
    Process {
        id: idleProc
        command: ["bash", "-c", "systemctl --user is-active hypridle >/dev/null 2>&1 && echo ON || echo OFF"]
        running: false
        stdout: StdioCollector {
            // idle ON → normal idle; OFF → stay-awake mode (show icon)
    ...
    Process { id: toggleProc; command: ["bash", "-c", "omaniri toggle idle"] }
