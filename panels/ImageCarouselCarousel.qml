...
PanelWindow {
    ...
    WlrLayershell.namespace: "omaniri-image-carousel-cr"
    ...
    Process {
        ...
        command: panel.isThemeMode
            ? ["bash", "-c",
               ...
               "name=$(cat ~/.config/omaniri/current/theme.name 2>/dev/null || true); " +
               "for ext in png jpg jpeg webp; do f=\"$CACHE/$name.$ext\"; [ -L \"$f\" ] && echo \"$f\" && exit 0; done; echo ''"]
            : ["bash", "-c", "readlink -f ~/.config/omaniri/current/background 2>/dev/null || true"]
        running: false
    ...
    function buildScanCmd() {
        if (isThemeMode) {
            return ["bash", "-c", [
                ...
                "for d in ~/.local/share/omaniri/themes/* ~/.config/omaniri/themes/*; do",
                "  [ -d \"$d\" ] || continue;",
                "  name=$(basename \"$d\");",
                "  prev=\"\";",
            ...
            return ["bash", "-c",
                "find -L ~/.config/omaniri/current/theme/backgrounds -maxdepth 1 -type f " +
    ...
    function applySelected() {
        ...
        if (isThemeMode) {
            ...
            applyThemeProc.command = ["bash", "-c", "omaniri-theme-set '" + name.replace(/'/g, "'\\''") + "'"]
            applyThemeProc.running = false; applyThemeProc.running = true
        } else {
            applyBgProc.command = ["bash", "-c", "omaniri-theme-bg-set '" + path.replace(/'/g, "'\\''") + "'"]
